defmodule PhoenixKitCustomerSupport.Web.UserDetails do
  ...
  defp maybe_allow_comment_upload(socket, true) do
    allow_upload(socket, :comment_attachments,
      ...
      auto_upload: true,
      progress: &handle_comment_upload_progress/3
    )
  end

  defp maybe_allow_comment_upload(socket, false), do: socket

  defp handle_comment_upload_progress(:comment_attachments, entry, socket) do
    if entry.done? do
      {:noreply, consume_done_comment_entry(socket, entry)}
    else
      {:noreply, socket}
    end
  end
    ...
    case Uploads.consume_entry(socket, entry, socket.assigns.current_user, "Comment") do
      {:ok, file} ->
        ...
        |> assign(
          ...
          socket.assigns.pending_comment_file_uuids ++ [file.uuid]
        )
