...
This module owns and versions the future shape of its 4 tables (`phoenix_kit_tickets`, `phoenix_kit_ticket_comments`, `phoenix_kit_ticket_attachments`, `phoenix_kit_ticket_status_history`) through its own migration chain, `PhoenixKitCustomerSupport.Migrations`. Core's `V135`/`V164`/`V168` still CREATE these 4 tables on every install — this chain's `V1` is a pure adoption of that exact shape: it stamps a version marker and changes nothing. The `~> 2.4` floor above exists precisely because `V1` must already include V164 (the `changed_by_uuid` nullability relaxation) and V168 (the unique slug index) for the adopted shape to match.
